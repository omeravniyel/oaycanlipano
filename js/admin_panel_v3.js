...
function previewAnnouncement() {
    ...
    if (!text) return Swal.fire('Uyarııı', 'Önizleme için metin giriniz', 'warning');
...
async function initSupabase() {
    ...
    try {
        ...
        if (typeof Swal !== 'undefined') Swal.fire('Hata', 'Veritabanı bağlantısı kurulamadı (Supabase SDK)', 'error');
...
async function doLogin() {
    ...
    try {
        ...
        if (!val) {
            ...
            if (typeof Swal !== 'undefined') Swal.fire('Uyarııı', 'Lütfen şifre giriniz.', 'warning');
...
function renderHadithWeeks() {
    ...
    // Fixed first week date: 08.09.2025 (September 8, 2025)
    if (!startDate) {
        startDate = '2025-09-08'; // Sabit ilk hafta tarihi
...
async function startDistributionUpload() {
    ...
    if (distributionTarget === 'region') {
        ...
        if (!distributionValue) { Swal.fire('Uyarııı', 'Mıntıka adı giriniz.', 'warning'); return; }
    ...
    if (distributionTarget === 'institution') {
        ...
        if (!distributionValue) { Swal.fire('Uyarııı', 'Kurum slug veya adı giriniz.', 'warning'); return; }
...
async function bulkDeleteSelected() {
    ...
    if (checkboxes.length === 0) {
        ...
        Swal.fire('Uyarııı', 'Silmek için önce görsel seçin', 'warning');
...
async function broadcastAnnouncement() {
    ...
    if (!text) return Swal.fire('Uyarııı', 'Duyuru metni giriniz', 'warning');
...
async function changePassword() {
    ...
    if (!currentPassword || !newPassword || !confirmPassword) {
        ...
        return Swal.fire('Uyarııı', 'Tüm alanları doldurunuz', 'warning');
    ...
    if (newPassword.length < 6) {
        ...
        return Swal.fire('Uyarııı', 'Yeni şifre en az 6 karakter olmalıdır', 'warning');
    ...
    if (newPassword !== confirmPassword) {
        ...
        return Swal.fire('Uyarııı', 'Yeni şifreler eşleşmiyor', 'warning');
